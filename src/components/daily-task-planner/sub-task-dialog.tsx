...
export function SubTaskDialog({
  ...
  useEffect(() => {
    if (editingSubtask) {
      const normalized = {
        ...editingSubtask,
        completedAt: editingSubtask.completedAt
          ? toDateTimeLocalString(editingSubtask.completedAt)
          : "",
      };
      reset(normalized);
    } else {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Subtasks belong to a parent task and can have their own shared
            contacts.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              defaultChecked={watch("isCompleted")}
            />
            {watch("isCompleted") && (
              <DateInput
                value={
                  (watch("completedAt") as string) ||
