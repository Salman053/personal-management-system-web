...
export function IncomeManager({ transactions, onEdit, onDelete, onAdd }: IncomeManagerProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">Rs. {monthlyIncome.toLocaleString()}</div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Badge variant="secondary">{Object.keys(incomeByCategory).length}</Badge>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{Object.keys(incomeByCategory).length}</div>
                ...
                <div key={category} className="flex items-center justify-between p-3 border rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {incomeTransactions.filter((t) => t.category === category).length} transactions
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-green-600">Rs. {amount?.toLocaleString()}</p>
                  </div>
                </div>
              ))}
          </div>
